...
# Crear la carpeta Longley si no existe
if (!dir.exists("Longley")) {
  dir.create("Longley")
}

# Aplicar la función a cada variable del dataset longley
library(ggplot2)
library(gridExtra)

lapply(names(longley), function(var) {
  # Obtener las medidas para la variable actual
  stats <- calc_stats(longley[[var]])
  ...
  ggsave(file.path("Longley", paste0(var, "_central.png")), tabla_central, width = 6, height = 1.5, units = "in")
  
  # Crear y guardar la tabla para medidas de dispersión
  tabla_dispersion <- tableGrob(tabla_dispersion_df, rows = NULL)
  ggsave(file.path("Longley", paste0(var, "_dispersion.png")), tabla_dispersion, width = 6, height = 1.5, units = "in")
})
